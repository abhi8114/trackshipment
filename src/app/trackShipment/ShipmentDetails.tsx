...
const ShipmentDetails = () => {
  ...
  const trackingData: TrackingData = {
    ...
    consigneeInfo: {
      ...
      address:
        "kasavanhalli radu pudur, Vlllar to Chokkalin Road, Kundrathpattu",
    },
    bookingInfo: {
    ...
    activityLog: [
      {
        ...
        update:
          "Shipment still not traceable till now and if the shipment not traceable by tomorrow marking it lost.",
      },
      {
        status: "In-Transit",
  ...
  const shipmentData: ShipmentData = {
    ...
    consigneeAddress:
      "kasavazha nadu pudur, Villar to Chokkalai Road, Kundithampattu",
  ...
  return (
    <div className="py-10 md:pb-16 pb-14 md:py-8 px-4 md:px-8 font-manrope text-primary max-w-[1400px] mx-auto">
      <p className="font-medium text-lg md:text-xl">
        Tracking ID: {trackingData.trackingId}
      </p>

      <div className="flex justify-between flex-col-reverse lg:flex-row mt-6">
        <div className="w-full lg:w-[836px] md:mr-5">
          {/* First Section */}
          <div className="shadow-md rounded-lg flex flex-col items-center justify-center w-full">
            {/* Header Row */}
            <div className="grid w-full grid-cols-1 md:grid-cols-6 bg-[#F8F9FF] text-center font-semibold">
              {[
                ...
                <div
                  key={index}
                  className={`p-4 text-sm md:text-base border-b border-[#DCDCDC] ${
                    index === 5 ? "" : "border-r"
                  } text-[#272727]`}
                >
                  {header}
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 bg-white text-center shadow-sm rounded-b-lg w-full">
              {[
                ...
                <div
                  key={index}
                  className={`p-4 text-[13px] md:text-sm text-[#272727] ${
                    index === 5 ? "" : "border-r border-[#DCDCDC]"
                  }`}
                >
                  {
                    trackingData.shipmentInfo[
                      key as keyof typeof trackingData.shipmentInfo
                    ]
                  }
                </div>
              ))}
          ...
          {/* Second Section */}
          <div className="mt-4 p-7 shadow-md rounded-lg bg-white w-full">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-x-28">
              {/* Row 1 */}
              <div>
                <h3 className="text-[#272727] font-semibold text-[14px] mb-2">
                  Consignee Name
                </h3>
          ...
          {/* Third Section */}
          <div className="mt-4 shadow-md rounded-lg bg-white w-full overflow-x-auto font-manrope">
            <table className="w-full text-sm md:text-base">
              <thead className="bg-[#F8F9FF]">
                <tr>
                  <th className="md:text-[15px] text-sm px-4 py-3 md:w-[25%] text-left">
                    Activity Status
                  </th>
                  <th className="md:text-[15px] text-sm px-4 py-3 md:w-[30%] text-left">
                    Date & Time
                  </th>
                  <th className="md:text-[15px] text-sm px-4 py-3 md:w-[55%] text-left">
                    Detailed Update
                  </th>
                </tr>
              </thead>
              <tbody>
                {trackingData.activityLog.map((log, index) => (
                  <tr key={index} className="border-b">
                    <td className="md:text-[15px] text-sm px-4 py-3">{log.status}</td>
                    <td className="md:text-[15px] text-sm px-4 py-3">{log.dateTime}</td>
                    <td className="md:text-[15px] text-sm px-4 py-3">{log.update}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Progress bar */}
        <div className="w-full lg:w-[calc(100%-850px)]">
          <TrackingTimeline />
        </div>
      </div>
    </div>
